  ...
  public Command moveArmToPosition(Double position) {
    return run(
        () -> {
          ...
          Double result =  MathUtil.clamp(0.01*armP.calculate(armMotor1.getEncoder().getPosition(), target), -1 * ArmConstants.armVelocityLimit, ArmConstants.armVelocityLimit);
          SmartDashboard.putNumber("encoder pos", armMotor1.getEncoder().getPosition());
          armMotor1.set(result);
          
          SmartDashboard.putNumber("position", target);
        });
  }
